...
export default function WishlistUiTable({ dataWishlist }) {
  ...
  function createData(id, title, description, quantity, price, imageCover) {
    return {
      id,
      title,
      description,
  ...
  function EnhancedTableToolbar(props) {
    ...
    async function handleRemoveProducts(IDs) {
      await IDs.map(async (id) => {
        await axios
          .delete(`${import.meta.env.VITE_DOMAIN_NAME}/api/v1/wishlist/${id}`, {
            headers: {
              Authorization: getCookies.token,
            },
          })
          .then(async ({ data }) => {
            ...
            navigate("/");
            renderData ? setRenderData(false) : setRenderData(true);
            //   ======== Test ==============
            // console.log(res);
          })
          .catch((err) => {
